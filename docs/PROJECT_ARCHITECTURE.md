...
- **Intelligent Feedback:** The AI acts as a tutor, providing conversation replies, grammatical corrections, a short explanation and example when there is a mistake, and a performance score (0-100).
...
- Uses **SQLAlchemy** with **PostgreSQL** (sync `psycopg2` driver) in both dev and prod. Alembic manages schema evolution.
- **Models:**
  - `Conversation`: Tracks unique sessions and `long_term_context`.
  - `Message`: Stores the user message, AI reply, correction, explanation (in `hinglish_explanation`), example, score, and timestamps.
...
- **Prompting:** The AI acts as a tutor, instructed via `app/core/prompts.py` to return JSON containing `reply_text` (spoken), `correction` (displayed), `explanation` (short, max 20 words), `example` (one example sentence), and `score` (0-100).
...
- **Backends:**
  - **Chatterbox-Turbo (English):** Requires a reference WAV (`tts_audio_prompt_path`) to perform high-quality zero-shot voice cloning locally. Optional low-latency settings: bfloat16 on CUDA (`TTS_TURBO_USE_BFLOAT16`), configurable generate/prepare_conditionals params (temperature, top_p, top_k, repetition_penalty, exaggeration, cfg_weight), optional torch.compile (`TTS_TURBO_COMPILE_T3`) and max_cache_len when the model or a fork exposes them, and SDPA forcing (`TTS_FORCE_SDPA_ATTENTION`: wraps the transformer forward so `output_attentions=False`/`output_hidden_states=False` to avoid the manual attention fallback). For maximum throughput, a fork that sets these at the call site (e.g. [rsxdalv/chatterbox](https://github.com/rsxdalv/chatterbox) #127) can be used. The library's `max_new_tokens` (e.g. 1000) is internal; capping it (e.g. to 500) would require a fork or patch that exposes it. Long-running servers: if TTS throughput drops over time, consider clearing AlignmentStreamAnalyzer hooks or restarting workers; some forks (e.g. Chatterbox-TTS-Server) handle this cleanup.
...
The application uses SQLAlchemy as its ORM on top of **PostgreSQL** (sync `psycopg2` driver). The same database engine is used for local development, CI, and production. The data models are defined in `app/models/usage.py` and related modules (`user.py`, `admin.py`, `billing.py`, `live_session.py`, `social_session.py`). Schema evolution is managed by Alembic; run `alembic upgrade head` on a fresh database to create all tables.
...
- `id` (String(36), Primary Key): UUID4 string generated by the app.
...
- `correction` (Text, Nullable): Corrected sentence or phrase provided by the AI when the user made a mistake.
- `hinglish_explanation` (Text, Nullable): Short explanation of the mistake (max 20 words); populated from the LLM’s `explanation` field.
- `example` (Text, Nullable): One example sentence showing correct usage when the user made a mistake.
...
4. **LLM Generation:** Send history, new message, and context to Gemini. The model returns a structured JSON (reply_text, correction, explanation, example, score).
...
5. **Server-Sent Events (SSE):** The pipeline yields events back to the client continuously:
   ...
   - `metadata`: Full correction, explanation, example, score, and conversation ID (sent after the LLM finishes).
